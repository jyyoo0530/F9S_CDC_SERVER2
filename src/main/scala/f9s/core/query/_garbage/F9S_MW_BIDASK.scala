...
case class F9S_MW_BIDASK(var spark: SparkSession) {

  def mw_bidask(f9StatsRaw:DataFrame, ftrData: List[(String, DataFrame)]): DataFrame = {
    println("////////////////////////////////MW BIDASK: JOB STARTED////////////////////////////////////////")
    ///// DATA LOAD /////
    val FTR_OFER_CRYR = ftrData.find(_._1 == "FTR_OFER_CRYR").get._2
    val FTR_OFER_RTE = ftrData.find(_._1 == "FTR_OFER_RTE").get._2
    val FTR_OFER_LINE_ITEM = ftrData.find(_._1 == "FTR_OFER_LINE_ITEM").get._2
    val FTR_OFER = ftrData.find(_._1 == "FTR_OFER").get._2
    val MDM_PORT = ftrData.find(_._1 == "MDM_PORT").get._2
...
//    val FTR_OFER_RTE = spark.read.parquet(filePath + "/FTR_OFER_RTE")
//    val FTR_OFER_LINE_ITEM = spark.read.parquet(filePath + "/FTR_OFER_LINE_ITEM")
//    val FTR_OFER = spark.read.parquet(filePath + "/FTR_OFER")
//    val MDM_PORT = spark.read.parquet(filePath + "/MDM_PORT")
//    lazy val leftJoinSeq1 = Seq("tradeOfferNumber", "tradeOfferChangeSeq")
//    lazy val filterSeq = FTR_OFER.groupBy("OFER_NR").agg(max("OFER_CHNG_SEQ").as("OFER_CHNG_SEQ"))
//
//    lazy val cryrSrc = filterSeq.join(FTR_OFER_CRYR, Seq("OFER_NR", "OFER_CHNG_SEQ"), "left").select("OFER_NR", "OFER_CHNG_SEQ", "OFER_CRYR_CD")
