...
case class F9S_MW_HST(var spark: SparkSession, var pathSourceFrom: String,
  ...
  def mw_hst(): Unit ={
    ...
    lazy val tgData = F9S_MW_HST.withColumn("writeIdx", concat(col("marketTypeCode"), col("baseYearWeek"), col("rdTermCode"), col("containerTypeCode"), col("paymentTermCode"), col("polCode"), col("podCode"))).withColumn("rte_idx", concat(col("polCode"), col("podCode")))
    ...
    for (i <- idx.indices){
      tgData.filter(
        ...
        .write.mode("append").json(pathJsonSave+"/F9S_MW_HST"+"/"+marketTypeCode(i)+ "/" + baseYearWeek(i)+"/"+paymentTermCode(i)+"/"+rdTermCode(i)+"/"+containerTypeCode(i)+"/"+rte_idx(i))
    }
